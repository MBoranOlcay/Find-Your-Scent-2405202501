        ...
        <div className="mb-6 p-4 bg-amber-50 rounded-lg flex flex-wrap items-center gap-2">
          ...
          {searchTerm.trim() && (
            <span className="inline-flex items-center gap-1.5 bg-gray-200 text-gray-700 text-xs font-medium px-2.5 py-1 rounded-full">
              Ara: &quot;{searchTerm}&quot;
              <button onClick={() => setSearchTerm("")} className="ml-1 text-gray-500 hover:text-gray-700"><XIcon size={12} /></button>
            </span>
          )}
          {selectedBrands.map(brand => (
            <span key={brand} className="inline-flex items-center gap-1.5 bg-purple-100 text-purple-700 text-xs font-medium px-2.5 py-1 rounded-full">
              {brand}
              <button onClick={() => removeBrandFilter(brand)} className="ml-1 text-purple-500 hover:text-purple-700"><XIcon size={12} /></button>
            </span>
          ))}
          {selectedNotes.map(note => (
            <span key={note} className="inline-flex items-center gap-1.5 bg-teal-100 text-teal-700 text-xs font-medium px-2.5 py-1 rounded-full">
              {note}
              <button onClick={() => removeNoteFilter(note)} className="ml-1 text-teal-500 hover:text-teal-700"><XIcon size={12} /></button>
            </span>
          ))}
          {selectedFamilies.map(family => (
            <span key={family} className="inline-flex items-center gap-1.5 bg-pink-100 text-pink-700 text-xs font-medium px-2.5 py-1 rounded-full">
              {family}
              <button onClick={() => removeFamilyFilter(family)} className="ml-1 text-pink-500 hover:text-pink-700"><XIcon size={12} /></button>
            </span>
          ))}
          {(activeFilterCount > 0) && (
        ...
        {fullyFilteredPerfumes.length > visibleCount && (
          <div className="mt-12 text-center">
            <button
              ...
              className="inline-flex items-center justify-center gap-2 bg-amber-600 hover:bg-amber-700 text-white font-sans font-semibold py-3 px-8 rounded-lg shadow-md hover:shadow-lg transition-all duration-300"
            >
